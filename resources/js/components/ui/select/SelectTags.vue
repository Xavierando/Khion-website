<template>
    <v-sheet class="d-flex">
        <v-select v-if="prodotto" v-model="tagsListValues" :items="tagsNameList" label="Chips" chips multiple>
        ...
        <v-dialog v-model="dialog" max-width="400" persistent>
            <v-card prepend-icon="mdi-tag" title="Aggiungi un nuovo tag">
                <v-card-text>
                    <v-text-field v-model="inputTag" :rules="[rules.nonVuoto, rules.Unico]"></v-text-field>
                </v-card-text>
                <template v-slot:actions>
                    <v-spacer></v-spacer>

                    <v-btn @click="handleAnnullaAggiuntaTag">
                        Annulla
                    </v-btn>

                    <v-btn @click="handleAggiungiTag">
                        Aggiungi
                    </v-btn>
                </template>
            </v-card>
...
watch(tagsListValues, (newValue, oldValue) => {
    ...
    if (newValue.length > oldValue.length) {
        const nameTagToAdd = newValue.find((newTag) => !oldValue.some((oldTag) => oldTag === newTag))
    ...
    if (newValue.length < oldValue.length) {
        const nameTagToRemove = oldValue.find((oldTag) => !newValue.some((newTag) => oldTag === newTag))
        const tagToRemove = (nameTagToRemove) ? tags.findByName(nameTagToRemove) : false
        if (tagToRemove) {
...
const handleAggiungiTag = () => {
    ...
    if (rules.nonVuoto(inputTag.value) === true && rules.Unico(inputTag.value) === true) {
