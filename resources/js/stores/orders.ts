...
export const useOrdersStore = defineStore("orders", {
                ...
                if (response.request.status === 200) {
                    if (response.data.message === "success") {
                        ...
                        for (const key in response.data.data.orders) {
                            this.list.push(
                                new Ordine(
                                    response.data.data.orders[key] as Order,
                                ),
                            );
                        }
                    }
            ...
            try {
                ...
                if (response.request.status === 200) {
                    if (response.data.message === "success") {
                        ...
                        this.list.push(
                            new Ordine(response.data.data.order as Order),
                        );
                    }
                }
            } catch (error) {
    ...
    getters: {
        all: (state) => state.list.sort(Ordine.sortByStatusAndDate),
        find: (state) => (orderId: number) =>
            state.list.find((order) => order.id === orderId) ?? false,
    },
});
...
export class Ordine implements IOrdine {
    ...
    static sortByStatusAndDate(OrdineA: Ordine, OrdineB: Ordine): number {
        const statusOrder = ["pending", "paid", "production", "expedited"];

        if (
            statusOrder.indexOf(OrdineA.status) -
                statusOrder.indexOf(OrdineB.status) !==
            0
        ) {
            return (
                statusOrder.indexOf(OrdineA.status) -
    ...
    async delete() {
        this.deleteFromServer().then(
            (isSuccess) => isSuccess && this.deleteFromLocalStore(),
        );
    }
    async deleteFromServer(): Promise<boolean> {
        try {
            const response = await axios.delete(
                "/api/orders/" + this.id.toString(),
            );
            if (response.request.status === 200) {
                if (response.data.message === "success") {
                    return true;
